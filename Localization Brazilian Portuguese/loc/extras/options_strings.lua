return {
	LocalizationBrazilianPortugueseOptionsButtonTitleID = "Configurações da Tradução PT-BR",
	LocalizationBrazilianPortugueseOptionsButtonDescID = "Configure a tradução ao seu gosto.",

	LocalizationBrazilianPortugueseCompatibilityOptionsButtonTitleID = "Compatibilidade",
	LocalizationBrazilianPortugueseCompatibilityOptionsButtonDescID = "Configure se a Tradução deveria ativar as traduções de alguns mods compatíveis.",

	LocalizationBrazilianPortugueseExtrasOptionsButtonTitleID = "Extras",
	LocalizationBrazilianPortugueseExtrasOptionsButtonDescID = "Opções que geralmente não afetará a sua jogatina.",

	LocalizationBrazilianPortugueseAprilFools2022TitleID = "Todo Dia é Primeiro de Abril de 2022",
	LocalizationBrazilianPortugueseAprilFools2022DescID = " ",

	LocalizationBrazilianPortugueseBRLCurrencyTitleID = "Dinheiro em Real",
	LocalizationBrazilianPortugueseBRLCurrencyDescID = "Deixe a Gangue Payday na pobreza!",

    LocalizationBrazilianPortugueseHopLibShouldBeInPortugueseTitleID = "Hoplib em português",
	LocalizationBrazilianPortugueseHopLibShouldBeInPortugueseDescID = "Decide se a tradução deveria forçar o Hoplib ficar em português.",

    LocalizationBrazilianPortugueseBeardLibShouldBeInPortugueseTitleID = "Beardlib em português",
	LocalizationBrazilianPortugueseBeardLibShouldBeInPortugueseDescID = "Decide se a tradução deveria forçar o Beardlib ficar em português.\n[AVISO: A TRADUÇÃO DO JOGO TAMBÉM DEPENDE DESSA OPÇÃO.]",
	}